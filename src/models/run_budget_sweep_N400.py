...
R = 8
N_TEST = 109
N_MATCH = int(os.environ.get("NMATCH", "150"))  # env-overridable; NMATCH=400 for the clean-slope within-cluster audit
...
for ti, (name, tau) in enumerate(TAU_LADDER):
    for r in range(R):
        ...
        kinds = []  # matched-only for the N=400 within-cluster audit (skip natural to save compute)
